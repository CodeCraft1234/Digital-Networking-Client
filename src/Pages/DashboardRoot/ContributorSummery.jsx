...
const ContributorSummery = ({ email }) => {
  ...
  // Data for the table (filtered by year)
  const employeeData = useMemo(() => {
    ...
    return relevantUsers.flatMap(user => {
      // Filter ads payments for the user by their email and selected year
      ...
      // Calculate total payment by month for the user
      ...
      return recentMonths.map(month => {
        // Filter monthly spent data by month and year
        ...
        // Filter sellery data by month and year
        const selleryData = (user.sellery || []).filter(sell => sell.month === month && sell.year === selectedYear);
  ...
  }, [users, email, adsPayment, recentMonths, selectedYear]); // Corrected dependencies
  ...
  }, [users, email, adsPayment, recentMonths]);
  ...
  //   return relevantUsers.flatMap(user => {
  //     const employeePayments = adsPayment.filter(payment => payment.employeeEmail === email);
  ...
  //       const monthlySpentData = (user.monthlySpent || []).filter(spent =>
  //         new Date(spent.date).toLocaleString('default', { month: 'long' })
  //       );
  //       const totalSpent = monthlySpentData.reduce((acc, spent) => acc + parseFloat(spent.totalSpentt), 0);
  //       const totalDollerRate = monthlySpentData.reduce((acc, spent) => acc + parseInt(spent.dollerRate), 0);
  //       const totalAdminPay = paymentByMonth[month] || 0;

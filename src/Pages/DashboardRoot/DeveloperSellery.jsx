...
<div className='f-between mb-4'>
    ...
    <div className="mb-4">
             ...
             <select
               ...
               name="employeeEmail"
             >
               {allEmployees?.filter(f=> f.role === 'webDeveloper').map((employee) => (
...
>
    ...
    {allEmployees
        .filter((u) => ['webDeveloper'].includes(u.role))
        .map((employee) => (
            <option key={employee._id} value={employee.email}>
                {employee.name}
            </option>
...
<tfoot className="font-bold">
  <tr className="tr1">
    ...
    <div className="bg-white pt-36 pb-12  font-sans lg:max-w-2xl lg:hidden mx-auto text-sm">
  {months.map((month) => {
      // Find the basic salary data for the selected employee and month
      const monthData = basicSalary?.find(
        (data) => data.employeeEmail === selectedEmployee && data.month === month
...
export default DeveloperSalary;

...
const YearlySummery = () => {
  ...
  <tbody className="text-black">
    {employeeData.map((user, index) => {
      ...
      const spendBDT = (totalSpent + totalPageSpent) * 142;
  ...
  <tr className="tr1">
    ...
    <td>৳ {employeeData.reduce((sum, user) => sum + ((parseFloat(user?.totalSpent || 0) + parseFloat(user?.totalPageSpent || 0)) * 142), 0).toLocaleString('en-IN')}</td>

    <td>৳ {employeeData.reduce((sum, user) => sum + parseFloat(user?.charge || 0), 0).toLocaleString('en-IN')}</td>
    <td>৳ {employeeData.reduce((sum, user) => sum + (((parseFloat(user?.totalSpent || 0) + parseFloat(user?.totalPageSpent || 0)) * 142)  + parseFloat(user?.charge || 0)), 0).toLocaleString('en-IN')}</td>
    <td>৳ {employeeData.reduce((sum, user) => sum + parseFloat(user?.adminPay || 0), 0).toLocaleString('en-IN')}</td>
    <td className="text-center">৳ {employeeData.reduce((sum, user) => sum + (((parseFloat(user?.totalSpent || 0) + parseFloat(user?.totalPageSpent || 0)) * 142) + parseFloat(user?.charge || 0) - parseFloat(user?.adminPay || 0)), 0).toLocaleString('en-IN')}</td>
  </tr>
</tfoot>

...
const GoogleMonthlySpend = () => {
  ...
  const initialTab =
  userr?.role === "admin"
  ? localStorage.getItem("a5") || "all" 
  : localStorage.getItem("a5") || user?.email; 
  ...
  {userr?.role === "admin" ? (
   ...
   <></>
      ...
      <div  className="overflow-x-auto rounded-xl  text-center " style={{ backgroundColor: 'var(--bg-color)', color: 'var(--text-color)'}}>
          ...
          <tbody >
            {sortedAccounts?.filter(f=>f.role === 'googleSpend').map((account, index) => (
